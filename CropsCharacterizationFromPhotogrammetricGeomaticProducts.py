...
def process(input_shp,
    ...
    for i in range(len(dsm_geotransform)):
        if abs(dsm_geotransform[i]-dtm_geotransform[i]) > 0.0000001:
            str_error = "Function process"
...
def main():
    ...
    if not options.input_crops_frames_shp:
        parser.print_help()
        return
    if not options.input_dsm:
        parser.print_help()
        return
    if not options.input_dtm:
        parser.print_help()
        return
    if options.crop_minimum_height == None:
        parser.print_help()
        return
    if not options.output_shp:
        parser.print_help()
        return
    if options.compute_GCC == None:
        parser.print_help()
        return
    if options.compute_volume == None:
        parser.print_help()
        return
    if options.date_from_dem_files == None:
        parser.print_help()
        return
    if not options.dem_files_string_separator:
        parser.print_help()
        return
    if not options.dem_files_date_string_position:
        parser.print_help()
        return
    ...
    if not date_from_dem_files:
        ...
        dsm_file_name_without_path = os.path.splitext(os.path.basename(input_dsm))[0]
        ...
        dtm_file_name_without_path = os.path.splitext(os.path.basename(input_dtm))[0]
    ...
    input_shp = input_crops_shapefile
    if not use_input_shp:
        str_error = copy_shapefile(input_crops_shapefile, output_shp)
        if str_error:
            print("Error:\n{}".format(str_error))
            return
        input_shp = output_shp
    str_error = process(input_shp,
                        input_dsm,
                        input_dtm,
                        crop_minimum_height,
                        compute_GCC,
